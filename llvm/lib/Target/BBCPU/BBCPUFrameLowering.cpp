//===-- BBCPUFrameLowering.cpp - BBCPU Frame Information ------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the BBCPU implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
void BBCPUFrameLowering::determineFrameLayout(MachineFunction &MF) const {
  MachineFrameInfo &MFI = MF.getFrameInfo();
  const BBCPURegisterInfo *LRI = STI.getRegisterInfo();
...
// ADJDYNALLOC pseudo instructions with a BBCPU:ADDI with the
// maximum call frame size as the immediate.
void BBCPUFrameLowering::replaceAdjDynAllocPseudo(MachineFunction &MF) const {
  // const BBCPUInstrInfo &LII =
  //     *static_cast<const BBCPUInstrInfo *>(STI.getInstrInfo());
  // unsigned MaxCallFrameSize = MF.getFrameInfo().getMaxCallFrameSize();

  // for (MachineBasicBlock &MBB : MF) {
  //   for (MachineInstr &MI : llvm::make_early_inc_range(MBB)) {
  //     if (MI.getOpcode() == BBCPU::ADJDYNALLOC) {
  //       DebugLoc DL = MI.getDebugLoc();
  //       Register Dst = MI.getOperand(0).getReg();
  //       Register Src = MI.getOperand(1).getReg();

  //       BuildMI(MBB, MI, DL, LII.get(BBCPU::ADD_I_LO), Dst)
  //           .addReg(Src)
...
void BBCPUFrameLowering::emitPrologue(MachineFunction &MF,
...
//   // Push old FP
//   // st %fp,-4[*%sp]
//   BuildMI(MBB, MBBI, DL, LII.get(BBCPU::SW_RI))
//       .addReg(BBCPU::FP)
//       .addReg(BBCPU::SP)
//       .addImm(-4)
...
//   // Generate new FP
//   // add %sp,8,%fp
//   BuildMI(MBB, MBBI, DL, LII.get(BBCPU::ADD_I_LO), BBCPU::FP)
//       .addReg(BBCPU::SP)
//       .addImm(8)
//       .setMIFlag(MachineInstr::FrameSetup);
...
//   // sub %sp,StackSize,%sp
//   if (StackSize != 0) {
//     BuildMI(MBB, MBBI, DL, LII.get(BBCPU::SUB_I_LO), BBCPU::SP)
//         .addReg(BBCPU::SP)
//         .addImm(StackSize)
//         .setMIFlag(MachineInstr::FrameSetup);
//   }
...
MachineBasicBlock::iterator BBCPUFrameLowering::eliminateCallFramePseudoInstr(
...
void BBCPUFrameLowering::emitEpilogue(MachineFunction & /*MF*/,
                                      MachineBasicBlock &MBB) const {
...
//   BuildMI(MBB, MBBI, DL, LII.get(BBCPU::ADD_I_LO), BBCPU::SP)
//       .addReg(BBCPU::FP)
//       .addImm(0);

//   // Restore the frame pointer from the stack.
//   BuildMI(MBB, MBBI, DL, LII.get(BBCPU::LDW_RI), BBCPU::FP)
//       .addReg(BBCPU::FP)
//       .addImm(-8)
//       .addImm(LPAC::ADD);
}

void BBCPUFrameLowering::determineCalleeSaves(MachineFunction &MF,
                                              BitVector &SavedRegs,
                                              RegScavenger *RS) const {
//   TargetFrameLowering::determineCalleeSaves(MF, SavedRegs, RS);

//   MachineFrameInfo &MFI = MF.getFrameInfo();
//   const BBCPURegisterInfo *LRI =
//       static_cast<const BBCPURegisterInfo *>(STI.getRegisterInfo());
//   int Offset = -4;
